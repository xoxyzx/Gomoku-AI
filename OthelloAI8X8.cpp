	...
	while(true)
	{
		...
		if(currentplayer==playerSide)
		{
			...
			while(true)
			{
				...
				if(tmpEnter.length()!=2 || (tmpEnter[0]-'A')<0 ||  (tmpEnter[0]-'A')>7 || (tmpEnter[1]-'a')<0 ||  (tmpEnter[1]-'a')>7)
...
{
	vector<pair<int, int> > ablemove;
	vector<vector<int> > tmpBoardArr(8);
